...
const InterviewPage = () => {
  ...
  const callGeminiAPI = async (payload) => {
    // Logging to help debug the 401 API key issue.
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            setApiError('Authentication Error (401): The API key is likely missing or invalid. Please check your setup.');
  ...
  const handleSendMessage = async (e) => {
    ...
    await getAIResponse(evaluationPrompt, chatHistory);
  };

  // Helper function to get AI response from Gemini API
  const getAIResponse = async (prompt, history = []) => {
    setApiError(null);
    const chatHistory = history.length > 0 ? history : [{ role: "user", parts: [{ text: prompt }] }];
    ...
    if (responseText) {
      ...
      setMessages(prevMessages => [...prevMessages, { sender: 'ai', text: 'I apologize, I could not generate a response. Please try again.' }]);
    }
    ...
    if (responseText) {
      try {
        ...
        if (Array.isArray(parsedData) && parsedData.length > 0) {
          ...
          // Handle case where API returns empty or malformed array
          setReviewData([{
